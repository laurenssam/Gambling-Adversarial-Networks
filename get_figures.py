...
if __name__ == '__main__':
    ...
    if opt.dataset == "camvid":
        train_set = camvid.Camvid(opt.phase, joint_transform=camvid.val_joint_transform,
    ...
    if opt.dataset == "cityscapes":
        ...
        focal = "focal_scratch_lrG1e-05_lrD00005_D1_L3_G-D50-100_ndf64_lamb10_GT00"
        ...
        el = "psp_cs_fin_early_lrG1e-06_lrD00001_D1_L3_G-D800-400_ndf128_lamb0005_GT00"
        HD = "psp_cs_HD_early_lrG5e-07_lrD00001_D2_L3_G-D800-400_ndf128_lamb001_GT00"
        pix = "psp_cs_early_lrG1e-06_lrD00001_D1_L3_G-D800-400_ndf128_lamb001_GT00"
        path = "Final_figures/PSP_cityscapes/"

    else:

        ## PSP_NET CAMVID
        seg = "psp075_scratch_lrG00001_lrD00005_D1_L3_G-D50-100_ndf64_lamb10_GT00"
        focal = "focal25_scratch_lrG00001_lrD00005_D1_L3_G-D50-100_ndf64_lamb10_GT00"
        v2 = "psp25_scratch_lrG5e-05_lrD5e-05_D1_L3_G-D100-200_ndf64_lamb10_GT00"
        v1 = "psp25_scratch_lrG5e-05_lrD5e-05_D1_L3_G-D100-200_ndf64_lamb05_GT00"
        el = "psp_el25_scratch_lrG25e-05_lrD00001_D1_L3_G-D150-150_ndf64_lamb01_GT00"
        HD = "psp5_HD_scratch_lrG25e-05_lrD00001_D2_L3_G-D100-200_ndf64_lamb0001_GT00"
        pix = "psp5_scratch_lrG5e-05_lrD00001_D1_L3_G-D100-200_ndf64_lamb0005_GT00"
        path = "Final_figures/PSP_camvid/"
    ...
    for model_inf in best_models:
        ...
        for i, data2 in enumerate(dataset):
            ...
            iou = evaluate(prediction.cpu().numpy(), model.real_B.cpu().numpy(), opt.output_nc)[2]
            ...
            if model_inf[0] == "seg":
                ...
                os.remove("temp.png")
                rgb = tensor2im(data2[0].cpu())
                GT = tensor2im(data2[1].cpu())
                rgb = Image.fromarray(rgb).convert("RGB")
                GT = Image.fromarray(GT).convert("RGB")
                filename_rgb = path + str(i) + "_rgb.png"
                filename_GT = path + str(i) + "_GT.png"
                filename_betmap = path + str(i) + "_betmap.png"
            ...
            image_path = path + str(i) + "_" + model_inf[0] + ".png"
            image_pil.save(image_path)
        ious.append(iou_per_model)
    best_images = []
    mean_improvements = []
    for i in range(len(ious[0])):
        delta_iou = [ious[-1][i] - ious[j][i]for j in range(len(best_models) - 1)]
        if all(k >= 0 for k in delta_iou):
            best_images.append(i)
            mean_improvements.append(np.mean(delta_iou))
    print(best_images)
    print(mean_improvements)
    test1, test2 = zip(*sorted(zip(mean_improvements, best_images)))
    print(test1, test2)
