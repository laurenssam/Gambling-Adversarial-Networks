...
class CESimpleModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            ...
            self.criterionBCE = torch.nn.BCEWithLogitsLoss(pos_weight=self.BCEweight, size_average=True, reduction="none")
    ...
    def get_borders(self, image):
        image[image == 255] = 0
        image = self.to_one_hot(image.unsqueeze(dim=1), 19).float().cpu().numpy()
        borders = []
        for i, index in enumerate(self.input_indices):
            size = image[i].shape # C X H X W
            border = torch.zeros(size[1], size[2]).to(self.device)
            for j in range(size[0]):
                border += torch.from_numpy(image[i, j] - scipy.ndimage.binary_erosion(image[i, j], iterations=self.opt.thickness).astype(np.int64)).float().to(self.device)
    ...
    def backward_D(self):
        ...
        fake_AB = torch.cat((self.real_A, self.fake_B_masked[:, 1].unsqueeze(dim=1)), 1)  # concat input rgb + masked output
        output_gambler = self.netD(fake_AB.detach()).squeeze()
        self.borders = self.get_borders(self.real_B.clone().detach())
        errors = ((torch.argmax(self.fake_B, dim=1) == 1) != (1 == self.real_B)).float() * (self.real_B != self.ignore).float()
        if self.iteration_D % 25 == 0:
            type_1 = (torch.argmax(self.fake_B, dim=1) == 1) * (1 != self.real_B) * (self.real_B != self.ignore)## GT is NOT pavement, Prediction is
            type_2 = (torch.argmax(self.fake_B, dim=1) != 1) * (1 == self.real_B) * (self.real_B != self.ignore) # GT is pavement, but fake is NOT
            bet_map = self.sigmoid(output_gambler.clone().detach())
            pos = bet_map > 0.5
            pos_type_1 = (pos * type_1).sum().item()
            pos_type_2 = (pos * type_2).sum().item()
            print("GT is NOT pavement, Prediction is: ", pos_type_1/pos.sum().item(), pos_type_1/type_1.sum().item(), type_1.sum().item()/errors.sum().item(), torch.mean(bet_map[type_1]).item())
            print("GT is pavement, but fake is NOT: ",   pos_type_2/pos.sum().item(), pos_type_2/type_2.sum().item(), type_2.sum().item()/errors.sum().item(), torch.mean(bet_map[type_2]).item())
            print("-" * 20)
        # bet_map[errors == 0] = 1- bet_map[errors == 0]
        # focal_weight = (1 - bet_map)**2
        self.loss_D_bet_loss = torch.mean(self.criterionBCE(output_gambler, errors) * (1-self.borders))  #self.criterionMSE(output_gambler, errors)
