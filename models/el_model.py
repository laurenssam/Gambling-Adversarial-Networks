...
class ELModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['EL', 'fit', 'D_EL', 'D_fake', 'D_real']
    ...
    def backward_D(self):
        ...
        # self.loss_D_EL = - self.criterionEL(emb_fake, emb_real) 
        # self.loss_D = self.loss_D_EL
    ...
    def backward_G(self):
        ...
        if self.pretrain:
            ...
            embeddings_fake = self.netD(fake_AB, emb=True)


            real_AB = torch.cat((self.real_A, self.real_B), 1)
            embeddings_real = self.netD(real_AB, emb=True)
            embeddings_real = embeddings_real.detach()

            self.loss_EL = (self.opt.lambda_EL/(self.opt.lambda_EL + 1)) *  self.criterionEL(embeddings_fake, embeddings_real)
    ...
    def optimize_G(self):
        ...
        # if self.pretrain == False:
        	...
        	# 	print(p.name, p.grad, p.grad.shape)
        	# quit()
