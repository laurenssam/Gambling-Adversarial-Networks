...
class ELGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_EL', type=float, default=100.0, help='weight for L1 loss')
    ...
    def backward_G(self):
        ...
        features_fake = self.netD(fake_AB, emb=True)

        with torch.no_grad():
            real_AB = torch.cat((self.real_A, self.real_B), 1)
            features_real = self.netD(real_AB, emb=True)

        # Second, G(A) = B
        self.loss_EL = self.criterionEL(features_fake[1], features_real[1])
        self.loss_G_L1 = self.criterionL1(self.fake_B, self.real_B) 
        # combine loss and calculate gradients
        self.loss_G = self.loss_EL + self.loss_G_L1
